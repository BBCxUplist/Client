...
const ReportsTab = ({ dummyReports }: { dummyReports: Report[] }) => {
  ...
  const filteredAndSortedReports = useMemo(() => {
    let filtered = dummyReports.filter((report) => {
      const matchesSearch = 
    ...
    return filtered;
  }, [dummyReports, searchTerm, filterType, filterStatus, filterPriority, sortBy, sortOrder]);
  ...
  return (
    ...
    >
      <div>
        ...
        <p className="text-white/70">Review and manage user reports and complaints</p>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 p-4 rounded-lg">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="lg:col-span-2">
            <label className="block text-white/70 text-sm mb-2">Search Reports</label>
            <input
              type="text"
              value={searchTerm}
      ...
      <div className="flex items-center justify-between">
        <p className="text-white/70">
          Showing {filteredAndSortedReports.length} of {dummyReports.length} reports
        </p>
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-white font-semibold">{report.reportedItem}</h4>
                  <p className="text-white/60 text-sm">Reported by {report.reportedBy}</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between text-sm">
              <span className="text-white/60">Reported on {new Date(report.createdAt).toLocaleDateString()}</span>
              <div className="flex gap-2">
      ...
      {filteredAndSortedReports.length === 0 && (
        <div className="text-center py-12">
          <p className="text-white/60 text-lg">No reports found matching your criteria</p>
          <button
            onClick={() => {
              setSearchTerm("");
              setFilterType("all");
