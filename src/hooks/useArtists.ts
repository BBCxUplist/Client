...
export const useArtistsByPriceRange = (minPrice: number, maxPrice: number): Artist[] => {
  const artists = useArtists();
  return artists.filter((artist) => 
    artist.price >= minPrice && artist.price <= maxPrice
  );
};
...
export const useArtistsByAvailability = (date: string): Artist[] => {
  const artists = useArtists();
  return artists.filter((artist) => 
    artist.availability.includes(date)
  );
};
...
export const useFilteredArtists = (filters: {
  ...
  if (filters.query) {
    filteredArtists = useSearchArtists(filters.query);
  }
  ...
  if (filters.minPrice !== undefined) {
    filteredArtists = filteredArtists.filter((artist) => artist.price >= filters.minPrice!);
  }
  
  if (filters.maxPrice !== undefined) {
    filteredArtists = filteredArtists.filter((artist) => artist.price <= filters.maxPrice!);
  }
  
  if (filters.minRating !== undefined) {
    filteredArtists = filteredArtists.filter((artist) => artist.rating >= filters.minRating!);
  }
