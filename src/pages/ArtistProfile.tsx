...
const ArtistProfile = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "about" | "music" | "reviews" | "gallery" | "booking"
  >("about");
  ...
  return (
    <div className="min-h-screen ">
      ...
      <div className="w-full p-4 md:p-6 lg:p-8 pb-24 md:pb-4">
        {/* Artist Header */}
        <div className="border-b border-dashed border-white pb-6 md:pb-8 lg:pb-10 mb-4 ">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="lg:col-span-2 flex flex-col justify-between">
              <div>
                <h1 className="font-mondwest text-4xl md:text-6xl lg:text-8xl font-bold text-white mb-4">
                  {artist?.name}
                </h1>
                   ...
                   {artist?.categories?.map((category, index) => (
                     <span
                       key={index}
                       className="bg-white/10 text-white px-4 py-2 text-sm border border-white/20"
                     >
                       {category}
                     </span>
              ...
              <div className="bg-white/5 p-6 border border-white/10">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    ...
                    <p className="text-2xl md:text-3xl font-bold text-orange-500 font-mondwest">
                      {formatPrice(artist?.price)}
                    </p>
                  </div>
                  <button className="bg-orange-500 text-black px-6 py-3 font-semibold hover:bg-orange-600 transition-colors duration-300 text-sm md:text-base">
                    BOOK NOW
                  </button>
                </div>
              </div>
        ...
        <div className="flex flex-wrap gap-4 mb-6 md:mb-8 border-b border-dashed border-white pb-4">
          {["about", "music", "reviews", "gallery", "booking"].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab as "about" | "music" | "reviews" | "gallery" | "booking")}
